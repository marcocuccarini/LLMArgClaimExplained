...
def extract_first_true_false(text):
    ...
    if m: return "TRUE" if m.group(1).lower()=="true" else "FALSE"
    return None

def parse_prediction(answer,prompt_type):
    if not answer: return "INVALID"
...
def run_experiment(K, MODEL, prompt_name, prompt_template, prompt_type, save_dir="results"):
    ...
    for idx,(claim,contexts,gt) in enumerate(zip(data["claims"],data["contexts"],data["answers"])):
        ...
        results.append({"index":idx,"claim":claim,"prediction":pred,"ground_truth":gt.strip().upper(),"model":MODEL,"K":K,"prompt":prompt_name})
        if idx%10==0 and idx>0:
            with open(predictions_file,"w") as f: json.dump(results,f,indent=2)
    with open(predictions_file,"w") as f: json.dump(results,f,indent=2)
    ...
    # Save summary
    summary = [{"prompt":prompt_name,"model":MODEL,"K":K,"accuracy":acc,"f1_score":f1}]
